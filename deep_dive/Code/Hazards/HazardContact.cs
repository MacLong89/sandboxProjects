...
public sealed class HazardContact : Component
{
	...
	public void Setup( HazardKind kind, float damage, float spriteHeight, float hitRadius = 2.2f )
	{
		...
		var tex = kind switch
		{
			HazardKind.Mine => DeepDivePixelArt.Mine(),
			HazardKind.Jellyfish => DeepDivePixelArt.Jellyfish(),
			HazardKind.Puffer => DeepDivePixelArt.Puffer(),
			HazardKind.Angler => DeepDivePixelArt.Angler(),
			_ => Texture.White
		};

		if ( tex is not null && tex.IsValid() && tex != Texture.White )
		{
			DeepDiveSprites.SpawnTexture( GameObject, tex, spriteHeight, name: "Sprite" );
			return;
		}
