...
public sealed class CollectiblePickup : Component
{
	...
	private void TryCollect( DeepDiveGame game )
	{
		if ( Collected || Definition is null )
			return;
	...
	private void BuildVisual()
	{
		...
		if ( !string.IsNullOrEmpty( Definition.TexturePath ) )
			tex = DeepDivePixelArt.Load( Definition.TexturePath );

		if ( tex is not null && tex.IsValid() && tex != Texture.White )
		{
			_sprite = DeepDiveSprites.SpawnTexture( GameObject, tex, Definition.SpriteWorldHeight, name: "Sprite" );
			return;
		}
