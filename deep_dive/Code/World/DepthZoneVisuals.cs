...
public sealed class DepthZoneVisuals : Component
{
	...
	public void Apply( float depthMeters )
	{
		...
		DeepDiveGame.Instance?.DiveCamera?.SetBackground( DayNightVisuals.Apply( water, DeepDiveGame.Instance?.Clock ) );
		...
		{
			_lastZone = zone;
			if ( DeepDiveGame.Instance?.Phase == GamePhase.Diving )
			{
				...
				DeepDiveGame.Instance.ShowMessage( label, 1.8f );
			}
		}
	}
