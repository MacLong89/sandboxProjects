...
public sealed class SeabedTerrain : Component
{
	...
	{
		var fill = DeepPixelArt.SeabedFill();
		var ridge = DeepPixelArt.SeabedRidge();
		var chunk = DeepPixelArt.SeabedChunk();
		var abyss = DeepPixelArt.AbyssSilhouette();
		var rocks = DeepPixelArt.Rocks();
	...
	private void BuildAbyssParallax( Texture abyss, Texture rocks, float y, BalanceConfig balance )
	{
		...
		if ( abyss is not null && abyss.IsValid() && abyss != Texture.White )
		{
			...
			for ( var i = 0; i < plates; i++ )
			{
				...
				var sr = DeepSprites.SpawnTexture( root, abyss, height, name: "Sprite" );
		...
		for ( var i = 0; i < 5; i++ )
		{
			...
			var sr = DeepSprites.SpawnTexture( root, rocks, height, name: "Sprite" );
	...
	private void BuildSedimentMass( Texture fill, float y )
	{
		...
		{
			Log.Warning( "[DEEP] seabed_fill missing — seafloor under-mass will be thin." );
			return;
		}
		...
		for ( var i = 0; i < segments; i++ )
		{
			...
			var sr = DeepSprites.SpawnTexture( root, fill, height, name: "Sprite" );
	...
	private void BuildSandRidge( Texture ridge, float y )
	{
		...
		for ( var i = 0; i < ridges; i++ )
		{
			...
			var sr = DeepSprites.SpawnTexture( root, ridge, height, name: "Sprite" );
	...
	private void BuildSurfaceChunks( Texture chunk, float y )
	{
		...
		{
			Log.Warning( "[DEEP] seabed_chunk missing — surface shelves skipped." );
			return;
		}
		...
		for ( var i = 0; i < xs.Length; i++ )
		{
			...
			DeepSprites.SpawnTexture( root, chunk, height, name: "Sprite" );
		}
	}
}
