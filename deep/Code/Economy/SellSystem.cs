...
public sealed class SellSystem
{
		...
		float oxygenFractionRemaining )
	{
		...
		if ( run.ObjectiveRewardPending || DeepGame.Instance?.Objectives?.AllComplete == true )
		{
			var objs = DeepGame.Instance?.Objectives;
			if ( objs is not null )
			{
	...
	public DiveSummary CommitSuccessfulSale( DiveSummary preview, HaulInventory haul, PlayerProgressionData progression )
	{
		...
		DeepGame.Instance?.Objectives?.GrantShellReward( progression );
