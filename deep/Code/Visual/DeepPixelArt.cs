...
public static class DeepPixelArt
{
	...
	public static Texture Load( string path )
	{
		...
		catch ( System.Exception e )
		{
			if ( LoggedMissing.Add( path ) )
				Log.Warning( $"[DEEP Art] Failed to load '{path}': {e.Message}" );
		}

		if ( LoggedMissing.Add( path ) )
			Log.Warning( $"[DEEP Art] Missing '{path}' — using fallback." );
