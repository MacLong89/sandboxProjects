...
public sealed class DiverSwimAnimator : Component
{
	...
	private string _clip = DeepPixelArt.IdleAnimation;
	...
	public void PlayHarpoon( Vector3 targetWorld )
	{
		...
		_clip = DeepPixelArt.HarpoonAnimation;
	...
	protected override void OnStart()
	{
		...
		if ( _renderer.IsValid() )
			_renderer.PlayAnimation( DeepPixelArt.IdleAnimation );
	...
	private void UpdateClip( Vector3 velocity )
	{
		...
		{
			next = DeepPixelArt.IdleAnimation;
		}
		else
		{
			...
			if ( absZ > absX * VerticalDominance )
				next = velocity.z > 0f ? DeepPixelArt.SwimUpAnimation : DeepPixelArt.SwimDownAnimation;
			else
				next = DeepPixelArt.SwimAnimation;
		}
