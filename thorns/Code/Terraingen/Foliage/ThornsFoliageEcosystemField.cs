...
public sealed class ThornsFoliageEcosystemField
{
		...
		int seed )
	{
		...
		var maxTreeSlope = config.GetMaxTreeHeightmapSlope( field, terrain );
		...
		for ( int y = 0; y < h; y++ )
		{
			for ( int x = 0; x < w; x++ )
			{
				...
				raw[field.Index( x, y )] = ComputeRawForestPotential( height, slope, terrainConfig, maxTreeSlope );
			}
		}
	...
	static float ComputeRawForestPotential( float height, float slope, ThornsTerrainConfig terrain, float maxTreeSlope )
	{
		...
		var flat = 1f - Smooth( slope, 0.02f, maxTreeSlope * 1.2f );
