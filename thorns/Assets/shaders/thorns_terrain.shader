...
HEADER
{
	Description = "THORNS terrain splat with triplanar cliff sampling (fork of base terrain.shader)";
    DebugInfo = false;
}
...
PS
{
    ...
    #include "terrain/ThornsTerrainTriplanar.hlsl"
    ...
    /// Steep cliff faces lerp to triplanar sampling so vertical tears don't smear top-down UVs.
    /// </summary>
    void Terrain_Splat( in float3 localPos, in float3 geoNormal, in CompactTerrainMaterial material,
        out float3 albedo, out float3 normal, out float roughness, out float ao, out float metal )
    {
        float2 texUV = localPos.xy / 32.0;
        float cliffBlend = Thorns_CliffTriplanarBlend( geoNormal );
        ...
        Texture2D tBaseBcr = Bindless::GetTexture2D( baseMat.bcr_texid );
        Texture2D tBaseNho = Bindless::GetTexture2D( baseMat.nho_texid );
        float4 baseBcr = tBaseBcr.Sample( g_sAnisotropic, baseSampleUV );
        float4 baseNho = tBaseNho.Sample( g_sAnisotropic, baseSampleUV );

        if ( cliffBlend > 0.001 )
        {
            float4 triBcr = Thorns_SampleTriplanarBcr( tBaseBcr, localPos, geoNormal, baseMat.uvscale );
            float4 triNho = Thorns_SampleTriplanarNho( tBaseNho, localPos, geoNormal, baseMat.uvscale );
            baseBcr = lerp( baseBcr, triBcr, cliffBlend );
            baseNho = lerp( baseNho, triNho, cliffBlend );
        }
        ...
        if ( cliffBlend > 0.001 )
        {
            Texture2D tOverlayBcr = Bindless::GetTexture2D( overlayMat.bcr_texid );
            Texture2D tOverlayNho = Bindless::GetTexture2D( overlayMat.nho_texid );
            float4 triBcr = Thorns_SampleTriplanarBcr( tOverlayBcr, localPos, geoNormal, overlayMat.uvscale );
            float4 triNho = Thorns_SampleTriplanarNho( tOverlayNho, localPos, geoNormal, overlayMat.uvscale );
            overlayBcr = lerp( overlayBcr, triBcr, cliffBlend );
            overlayNho = lerp( overlayNho, triNho, cliffBlend );
        }
	...
	float4 MainPs( PixelInput i ) : SV_Target0
	{
        ...
        float3 localGeoNormal;

        // Calculate base normal from heightmap (local space — matches LocalPosition for triplanar)
        localGeoNormal = Terrain_Normal( tHeightMap, uv, Terrain::Get().HeightScale, tangentU, tangentV );
        float3 geoNormal = mul( Terrain::Get().Transform, float4( localGeoNormal, 0.0 ) ).xyz;
        ...
        if ( Terrain::Get().ControlMapTexture != 0 )
        {
            ...
            Terrain_Splat( i.LocalPosition, localGeoNormal, mat00, albedo00, normal00, rough00, ao00, metal00 );
            Terrain_Splat( i.LocalPosition, localGeoNormal, mat10, albedo10, normal10, rough10, ao10, metal10 );
            Terrain_Splat( i.LocalPosition, localGeoNormal, mat01, albedo01, normal01, rough01, ao01, metal01 );
            Terrain_Splat( i.LocalPosition, localGeoNormal, mat11, albedo11, normal11, rough11, ao11, metal11 );
        ...
        // THORNS: matte outdoor ground — albedo alpha often reads too glossy.
        roughness = max( roughness, 0.96 );
        metalness = 0;
