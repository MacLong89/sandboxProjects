...
/// Client never chooses final hit direction; owner view kick is smoothed in <see cref="Player.ThornsPlayerLocomotion.ApplyWeaponRecoilKick"/>.
/// </summary>
public static class ThornsWeaponRecoilSolve
{
	/// <summary>Global tuning on top of per-weapon <see cref="ThornsWeaponDefinitions.WeaponDefinition.RecoilPatternScaleDegrees"/> — scales authoritative kick and <see cref="ThornsWeaponDefinitions.WeaponDefinition.ClientVisualKickScale"/> together (melee skips the recoil path).</summary>
	...
	public static IReadOnlyList<Vector2> GetPatternSteps( ThornsWeaponDefinitions.WeaponDefinition def )
	{
		if ( def is null )
	...
	/// <see cref="ThornsWeaponDefinitions.WeaponDefinition.RecoilResetDelaySeconds"/> resets pattern/spray ordinal (tap / burst cadence — not instantaneous).
	/// </summary>
	public static Vector3 SolveAuthoritativeFireDirection(
		...
		float attachmentRecoilKickMul = 1f,
		float attachmentBloomBaseMul = 1f )
	{
		...
		if ( def is null || ThornsWeaponDefinitions.IsMeleeWeapon( def ) )
		...
		recoilMul *= attachmentRecoilKickMul;

		float bloomMul = 1f;
		bloomMul *= moving ? def.MovingBloomMul : 1f;
		bloomMul *= crouched ? def.CrouchBloomMul : 1f;
		bloomMul *= adsHeld ? def.AdsBloomMul : 1f;
		bloomMul *= attachmentBloomBaseMul;

		var yawKickDeg = step.x * def.RecoilPatternScaleDegrees * recoilMul * GlobalGunRecoilKickMul;
		var pitchKickDeg = step.y * def.RecoilPatternScaleDegrees * recoilMul * GlobalGunRecoilKickMul;
		...
		var bloomRequested =
			(def.BloomHalfAngleDegreesBase * attachmentBloomBaseMul + sprayOrdinalHost * def.BloomHalfAngleDegreesPerSprayShot) * bloomMul;
		...
		clientKickPitchDeg = pitchKickDeg * def.ClientVisualKickScale * GlobalVisualKickMul;
		clientKickYawDeg = yawKickDeg * def.ClientVisualKickScale * GlobalVisualKickMul;
		if ( adsHeld )
		{
			clientKickPitchDeg *= AdsVisualKickMul;
			clientKickYawDeg *= AdsVisualKickMul;
		}

		if ( crouched )
		{
			clientKickPitchDeg *= CrouchVisualKickMul;
			clientKickYawDeg *= CrouchVisualKickMul;
		}
