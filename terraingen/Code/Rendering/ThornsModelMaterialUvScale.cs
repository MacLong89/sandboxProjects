...
public static class ThornsModelMaterialUvScale
{
		...
		string modelAssetPathOverride = null )
	{
		...
		foreach ( var mr in root.Components.GetAll<ModelRenderer>( findMode ) )
		{
			...
			{
				var scale = mr.GameObject.IsValid() ? mr.GameObject.LocalScale : Vector3.One;
				ApplyForPlaceableFurniture( mr, mr.Model, path, scale );
			}
			else
		...
		bool isPlaceable = false )
	{
		...
		if ( isPlaceable )
		{
			ApplyPlaceableMaterial( renderer, src, uvScale, modelAssetPath, minScaleToCompensate );
			return;
		}
	...
	/// <summary>
	/// Use materials baked on the vmdl (ModelDoc default group). Runtime <see cref="Material.CreateCopy"/>
	/// + high <c>g_vTexCoordScale</c> was producing pink error shaders in the floorplan gallery.
	/// </summary>
	static void ApplyPlaceableMaterial(
		ModelRenderer renderer,
		Material src,
		Vector2 uvScale,
		string modelAssetPath,
		float minScaleToCompensate )
	{
		if ( renderer is null || !renderer.IsValid() )
			return;

		_ = src;
		_ = minScaleToCompensate;
		renderer.MaterialOverride = default;

		ThornsFurnitureMaterialDebug.LogMaterialStep(
			"PlaceableMat vmdl-only",
			modelAssetPath,
			renderer.GameObject.IsValid() ? renderer.GameObject.WorldScale : default,
			uvScale,
			needsUv: NeedsUvCompensation( uvScale, minScaleToCompensate ),
			src,
			renderer.MaterialOverride );
	}

	static bool IsUsableMaterial( Material mat ) => mat.IsValid();
		...
		string modelAssetPath )
	{
		...
		if ( !TryCreateScaledMaterialCopy( src, uvScale, modelAssetPath, out var copy ) || !IsUsableMaterial( copy ) )
			return;
		...
		out Material copy )
	{
		...
		if ( ScaledMaterialCopyCache.TryGetValue( cacheKey, out var cached ) && IsUsableMaterial( cached ) )
		{
			copy = cached;
			return true;
		}
	...
	public static Material LoadMaterialForModel( string modelAssetPath )
	{
		...
		if ( MaterialCache.TryGetValue( vmatPath, out var cached ) && IsUsableMaterial( cached ) )
			return cached;

		var mat = TryLoadMaterial( vmatPath, allowMaterialLoad: true );
		if ( IsUsableMaterial( mat ) )
	...
	static Material TryLoadMaterial( string vmatPath, bool allowMaterialLoad )
	{
		...
		if ( fromLibrary is { IsValid: true } )
		{
			ThornsFurnitureMaterialDebug.Write(
				$"TryLoad OK library path={trimmed} mat={ThornsFurnitureMaterialDebug.DescribeMaterial( fromLibrary )}" );
			return fromLibrary;
		}
		...
		if ( ThornsFurnitureMaterialDebug.Enabled )
		{
			ThornsFurnitureMaterialDebug.Write(
				loaded.IsValid()
					? $"TryLoad OK Material.Load path={trimmed} mat={ThornsFurnitureMaterialDebug.DescribeMaterial( loaded )}"
					: $"TryLoad FAIL Material.Load path={trimmed}" );
		}
