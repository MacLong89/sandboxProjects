...
/// <summary>
/// Non-throwing Model/Material loads with once-per-path warnings. Never invent paths — catalog first.
/// </summary>
public static class AssetSafe
{
	...
	private static void WarnOnce( string path, string detail )
	{
		...
		Log.Warning( $"[SceneKit] Asset unavailable '{path}' ({detail}) — using fallback if available." );
	}
}
