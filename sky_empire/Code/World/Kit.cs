...
public static class Kit
{
	...
	public static GameObject Box( GameObject parent, string name, Vector3 localPos, Vector3 size, Color tint, bool collider = false )
	{
	...
	/// <summary>Puffy decorative cloud (2-4 lobes), no collider.</summary>
	public static void Cloud( GameObject parent, Vector3 pos, float scale, Random rng )
	{
		var root = new GameObject( true, "Cloud" );
		root.SetParent( parent );
		root.LocalPosition = pos;
		root.LocalRotation = Rotation.FromYaw( (float)rng.NextDouble() * 360f );
		var white = new Color( 1f, 1f, 1f, 0.92f );

		var lobes = 2 + rng.Next( 3 );
		for ( int i = 0; i < lobes; i++ )
		{
			var off = new Vector3( (i - lobes * 0.5f) * 65f * scale, ((float)rng.NextDouble() - 0.5f) * 50f * scale, ((float)rng.NextDouble() - 0.5f) * 22f * scale );
			var s = (0.7f + (float)rng.NextDouble() * 0.6f) * scale;
			Sphere( root, $"Puff{i}", off, new Vector3( 150f, 120f, 70f ) * s, white );
		}
	}
}
